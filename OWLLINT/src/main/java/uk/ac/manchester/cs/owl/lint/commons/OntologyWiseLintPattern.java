 ...
 *         Feb 15, 2008
 */
public abstract class OntologyWiseLintPattern<O extends OWLObject>
        implements LintPattern<O> {
    ...
    public PatternReport<O> matches(Collection<? extends OWLOntology> targets)
        ...
        Set<Match<O>> set = new HashSet<>(targets.size());
        for (OWLOntology ontology : targets) {
            set.addAll(this.matches(ontology));
        }
        return new SimpleMatchBasedPatternReport<>(this, set);
    }
