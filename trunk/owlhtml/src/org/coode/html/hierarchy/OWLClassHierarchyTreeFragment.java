...
public class OWLClassHierarchyTreeFragment extends AbstractTreeFragment<OWLClass> {
    ...
    public void setFocus(OWLClass focusClass){
        this.focusClass = focusClass;
        refreshRequired = true;
    }

    public OWLClass getFocus() {
        return focusClass;
    }
    ...
    public List<OWLClass> getChildren(OWLClass parent) {
        if (refreshRequired){
            refresh();
        }
    ...
    private void refresh() {
        clear();
        try {
            generateAncestorHierarchy(focusClass, 0);
            generateDescendantHierarchy(focusClass, 0);
        }
    ...
    private void generateDescendantHierarchy(OWLClass currentCls, int depth) throws OWLReasonerException {
        if (descendantLevels < 0 || depth < descendantLevels){
    ...
    private void generateAncestorHierarchy(OWLClass currentCls, int depth) throws OWLReasonerException {
        if (depth < ancestorLevels && !currentCls.equals(getOWLThing())){
