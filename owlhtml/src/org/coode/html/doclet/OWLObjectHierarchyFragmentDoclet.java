...
public class OWLObjectHierarchyFragmentDoclet<O extends OWLNamedObject> extends AbstractHTMLDoclet<O> {
    ...
    private static final Logger logger = Logger.getLogger(OWLObjectHierarchyFragmentDoclet.class);
    ...
    private void printExpandLink(O node, PrintWriter out) {
        try {
            if (renderExpandLinks){
                ...
                URL url = server.getURLScheme().getURLForNamedObject(node);
            ...
            logger.error("Could not print expand link: " + node.getURI(), e);
        }
    }
}
